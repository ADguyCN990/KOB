...
public class WebSocketServer {
    ...
    //线程安全的匹配池
    public Game game = null; //随机生成的游戏地图，要返回给前端
    @Autowired
   ...
   private void stopMatching() {
    ...
    private void move(int direction) {
        if (user.getId().equals(game.getPlayerA().getId()) && game.getPlayerA().getBotId().equals(-1)) {
            game.setNextStepA(direction);
        }
        else if (user.getId().equals(game.getPlayerB().getId()) && game.getPlayerB().getBotId().equals(-1)) {
